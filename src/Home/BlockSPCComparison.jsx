...
export default function BlockSPCComparison() {
  ...
  function downloadCSV() {
    const header = ["Block SPC Advantage","Typical Exchange / Provider"].join(",");
    const body = filtered
      .map((r) => `"${r.title.replace(/"/g, '""')}","${r.typical.replace(/"/g, '""')}"`)
      .join("\n");
    const csv = header + "\n" + body;
    ...
    a.href = url;
    a.download = "blockspc-comparison.csv";
    a.click();
          ...
          <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-6">
            <div>
              <h1 data-aos="zoom-in-up" data-aos-delay="300" data-aos-duration="400" className='text-left lg:text-4xl text-2xl md:pt-5 font-medium text-[#1A2B6B]'>
                Why Blocksphere is different?
              </h1>
              <p className="text-sm mt-3 text-slate-600"> <span className="text-sm font-semibold text-slate-900">BlockSphere</span> Vs Crypto Exchanges</p>
            </div>
          </header>
              ...
              <table className="w-full table-fixed border-collapse">
                <thead>
                  <tr className="bg-slate-100">
                    <th className="w-1/2 text-left p-4 text-sm md:text-base font-semibold text-[#1A2B6B]">Blocksphere Advantage</th>
